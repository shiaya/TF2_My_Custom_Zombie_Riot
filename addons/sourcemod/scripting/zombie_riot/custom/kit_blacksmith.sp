...
void Blacksmith_BuildingUsed_Internal(int weapon ,int entity, int client, int owner, bool reset)
{
	...
	else
	{
		...
		else if(i_CustomWeaponEquipLogic[weapon] == WEAPON_MINECRAFT_SWORD && slot == TFWeaponSlot_Melee)
		{
			switch(SmithLevel[owner])
			{
				case 0:
	...
	switch(tinker.Rarity)
	{
		...
		case 3:
		{
			ClientCommand(client, "playgamesound baka/blacksmith_tick_quad.mp3");
		}
		case 4:
		{
			ClientCommand(client, "playgamesound baka/blacksmith_tick_five.mp3");
		}
	}
	...
	if(!Rogue_Mode() && owner != client)
	{
		switch(tinker.Rarity)
		{
			...
			case 3:
			{
				ClientCommand(owner, "playgamesound baka/blacksmith_tick_quad_friend.wav");
			}
			case 4:
			{
				ClientCommand(owner, "playgamesound baka/blacksmith_tick_five_friend.wav");
			}
		}
	}
}
...
void Blacksmith_PrintAttribValue(int client, int attrib, float value, float luck, bool addition = false, int CustomMode = 0)
{
	...
	if(addition)
	{
		FormatEx(buffer, sizeof(buffer), "%.1f ", value);
	}
	else if(value < 1.0)
	{
	...
	switch(attrib)
	{
		...
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sSweeping Edge Max Target", buffer);
			else
		...
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sSweeping Edge Charge", buffer);
		}
		...
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sSweeping Edge Range", buffer);
			else
		...
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sSec On Hit Ignite", buffer);
		}
		
		case 410:
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sJump Crit Damage", buffer);
			else
				Format(buffer, sizeof(buffer), "%sBase Damage", buffer);
		}
		case 411:
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sSec On Hit Silenced", buffer);
		}
		...
		{
			if(CustomMode==1)
				Format(buffer, sizeof(buffer), "%sSweeping Edge Damage", buffer);
		}
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Sharpness");
	tinker.Attrib[0] = 2;
	float DamageLuck = (0.1 * (tinker.Luck[0]));
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Smite");
	tinker.Attrib[0] = 2;
	tinker.Attrib[1] = 410;
	tinker.Attrib[2] = 41;
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Sweeping Edge");
	tinker.Attrib[0] = 99;
	tinker.Attrib[1] = 4;
	tinker.Attrib[2] = 425;
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Quick Charge");
	tinker.Attrib[0] = 41;
	tinker.Attrib[1] = 6;
	tinker.Attrib[2] = 425;
	tinker.CustomMode[0]=1;
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Bane of Arthropods");
	tinker.Attrib[0] = 2;
	tinker.Attrib[1] = 411;
	tinker.Addition[1]=true;
	tinker.CustomMode[1]=1;
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Fire Aspect");
	tinker.Attrib[0] = 2;
	tinker.Attrib[1] = 397;
	tinker.Addition[1]=true;
	tinker.CustomMode[1]=1;
...
{
	strcopy(tinker.Name, sizeof(tinker.Name), "Curse of Glassy");
	tinker.Attrib[0] = 2;
	tinker.Attrib[1] = 425;
	tinker.Attrib[2] = 205;
	tinker.Attrib[3] = 206;
