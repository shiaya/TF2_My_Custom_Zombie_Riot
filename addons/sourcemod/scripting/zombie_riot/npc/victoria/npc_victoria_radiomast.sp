...
public void VictoriaRadiomast_ClotThink(int iNPC)
{
	...
	{
		if((npc.m_flNextMeleeAttack - gameTime)>InfiniteWave)
			RaidModeScaling = 1972.0;
		else
			RaidModeScaling = ((InfiniteWave-(npc.m_flNextMeleeAttack - gameTime))/InfiniteWave)*1972.0;
	}
	if(Waves_IsEmpty())
	{
		int ISVOLI= 1;
