...
void VictoriaSpecialBigPipe_OnMapStart_NPC()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_special_bigpipe");
...
{
	return VictoriaSpecialBigPipe(client, vecPos, vecAng, ally);
}

methodmap VictoriaSpecialBigPipe < CClotBody
{
	...
	public VictoriaSpecialBigPipe(int client, float vecPos[3], float vecAng[3], int ally)
	{
		VictoriaSpecialBigPipe npc = view_as<VictoriaSpecialBigPipe>(CClotBody(vecPos, vecAng, "models/player/demo.mdl", "1.0", "1250", ally,false));
		...
		func_NPCDeath[npc.index] = view_as<Function>(VictoriaSpecialBigPipe_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VictoriaSpecialBigPipe_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(VictoriaSpecialBigPipe_ClotThink);
...
public void VictoriaSpecialBigPipe_ClotThink(int iNPC)
{
	VictoriaSpecialBigPipe npc = view_as<VictoriaSpecialBigPipe>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		int ActionDo = VictoriaSpecialBigPipeSelfDefense(npc,GetGameTime(npc.index), flDistanceToTarget); 
		switch(ActionDo)
		{
			case 0:
			{
...
public Action VictoriaSpecialBigPipe_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VictoriaSpecialBigPipe npc = view_as<VictoriaSpecialBigPipe>(victim);
...
public void VictoriaSpecialBigPipe_NPCDeath(int entity)
{
	VictoriaSpecialBigPipe npc = view_as<VictoriaSpecialBigPipe>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
int VictoriaSpecialBigPipeSelfDefense(VictoriaSpecialBigPipe npc, float gameTime, float distance)
{
	//Direct mode
